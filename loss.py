...
def compute_loss(model, X_r, X_data, u_data, X_mc, u_mc_targets, k, theta, sigma, lambda_jump, jump_std, epoch=0):
    """
    ...
    Returns:
        tuple: Tuple containing (total_loss, loss_interior, loss_terminal, loss_boundary, loss_mc)
    """
    ...
    # Monte Carlo anchor point loss
    u_mc_pred = model(X_mc)
    loss_mc = torch.mean((u_mc_targets - u_mc_pred) ** 2)

    # Compute total loss
    total_loss = (loss_interior + 
                  loss_terminal + 
                  loss_boundary + 
                  loss_mc)

    return total_loss, loss_interior, loss_terminal, loss_boundary, loss_mc 