...
% \section{Chapter Objectives and Structure}
% \label{sec:chapter_objectives}

% This introductory chapter aims to provide the necessary mathematical foundations for understanding Lévy processes and their application in the context of structural credit risk modeling. The objective is to build the theory rigorously, starting from fundamental concepts of probability and stochastic processes, progressing through the essential theories of Brownian motion and Itô calculus, introducing Poisson processes as the simplest jump mechanism, and culminating in the general theory of Lévy processes, including their characterization and decomposition. We will also introduce Ornstein-Uhlenbeck (OU) processes, both Gaussian and Lévy-driven variants, which incorporate the important feature of mean reversion.

% The chapter will then illustrate how these advanced stochastic processes, particularly Lévy and Lévy-driven OU processes, are employed within structural credit risk models to determine the probability of default (PD). The focus will be on the conceptual framework and the mathematical tools required, rather than on specific model calibration or empirical testing, which may be addressed in subsequent chapters.

% The structure of this chapter is as follows:
% \begin{itemize}
%     \item Section \ref{sec:preliminaries} reviews essential mathematical preliminaries, including probability spaces, stochastic processes, filtrations, and stopping times.
%     \item Section \ref{sec:brownian_ito} introduces the cornerstone continuous process, Brownian motion, and the associated Itô calculus, including the fundamental Itô's Lemma.
%     \item Section \ref{sec:poisson} discusses the incorporation of jumps through Poisson and compound Poisson processes.
%     \item Section \ref{sec:levy_general} presents the general theory of Lévy processes, covering their definition, the property of infinite divisibility, the Lévy-Khintchine formula, and the Lévy-Itô decomposition.
%     \item Section \ref{sec:ou_processes} introduces Ornstein-Uhlenbeck processes, covering both the standard Gaussian case and the extension to Lévy-driven OU processes, highlighting the property of mean reversion.
%     \item Section \ref{sec:application_structural} outlines the application of these processes within structural credit risk models, focusing on modeling firm value and the calculation of default probabilities.
%     \item Section \ref{sec:summary_roadmap} provides a summary of the key concepts covered and outlines the connection to the subsequent chapters of this thesis.
% \end{itemize}
...
\end{itemize}
Therefore, the study of Lévy processes is intrinsically linked to the study of infinitely divisible distributions. Examples of infinitely divisible distributions include the Normal, Poisson, Compound Poisson, Gamma, Negative Binomial, Stable, Variance Gamma, and Normal Inverse Gaussian distributions.
...
% To make the example self-contained, let's define textsuperscript if not available
% (though usually provided by packages like textcomp or natively in newer engines)
...
The central quantity of interest is the probability of default by time $t$, given the information at time 0 (including the initial asset value $V_0 = x > K$):
\[ \text{PD}(x, t) = P(\tau \le t | V_0 = x) \]
Calculating this first hitting time probability for a general Lévy process is significantly more challenging than for Brownian motion due to the presence of jumps. The process can jump over the barrier, landing strictly below $K$ at the time of default. Analytical closed-form solutions for $\text{PD}(x, t)$ are typically unavailable except in very specific cases.

Several approaches are used to compute or approximate the PD:
\begin{itemize}
    \item \textbf{Partial Integro-Differential Equations (PIDEs):} The probability of default $\Phi(x, t) = P(\tau \le t | V_0 = x) = \text{PD}(x, t)$ often satisfies a PIDE related to the infinitesimal generator of the underlying Lévy process (or Lévy-driven OU process) governing $V_t$ or $\ln V_t$. For a process $X_t = \ln(V_t / K)$ with generator $\mathcal{L}$, the PIDE for the probability of default $\Phi(x, t)$ (where $x = \ln(V_0 / K) > 0$ and default occurs when $X_t \le 0$) typically takes the form: $\frac{\partial \Phi}{\partial t} = -\mathcal{L} \Phi$ subject to boundary conditions $\Phi(x, t) = 0$ for $x \le 0$ (defaulted state) and initial condition $\Phi(x, 0) = 1$ for $x > 0$. The generator $\mathcal{L}$ includes drift, diffusion, and integral terms reflecting the Lévy triplet $(a, \sigma^2, \nu)$.
    ...
    \item \textbf{Monte Carlo Simulation:} This involves simulating a large number of sample paths of the asset value process $V_t$ according to its Lévy dynamics. The probability $\text{PD}(x, t)$ is then estimated as the fraction of simulated paths that hit or cross the barrier $K$ at or before time $t$. While conceptually straightforward, Monte Carlo can be computationally intensive, especially for low default probabilities or complex Lévy processes.
\end{itemize}
...
Assuming time homogeneity in the underlying process dynamics allows us to define the probability of default primarily as a function of the initial asset value $x$ and the time horizon (time until maturity) $u$:
$$ \Phi(x, \dots, u) = P\left( \inf_{r \le u} G_r \le 0 | G_0 = x, \dots \right) $$
where `...` represents potential initial states of other latent variables driving the process.
...
% To incorporate continuous latent variables like macroeconomic factors influencing volatility, we model the volatility itself as a stochastic process $Y_u$ (e.g., a CIR process), affecting the diffusion term:
% $$ dG_u = k(\theta - G_u)du + \sqrt{Y_u} dB_u + \int_{\mathbb{R}} z N(du, dz), \quad G_0 = x $$
% $$ dY_u = \kappa(\mu - Y_u)du + \xi \sqrt{Y_u} dW_u, \quad Y_0 = y $$
% Here, $Y_u$ follows its own mean-reverting process, and $W_u$ is another Brownian motion, independent of $B_u$.

% \subsubsection{Generalized Model}
% Combining both extensions leads to a generalized model where parameters depend on both the regime $R_u$ and the stochastic volatility $Y_u$:
% $$ dG_u = k_{\rho}(\theta_{\rho} - G_u)du + \sigma_{\rho}\sqrt{Y_u} dB_u + \int_{\mathbb{R}} z N(du, dz), \quad G_0 = x, R_0 = \rho, Y_0 = y $$
% $$ dY_u = \kappa(\mu - Y_u)du + \xi \sqrt{Y_u} dW_u $$
% The survival probability is then $\Phi(x, \rho, y, u)$.
...
% $$ \Phi(x, y, u) = \int_0^u \lambda e^{-\lambda s} \int_0^\infty \int_{\mathbb{R}} T_s\Phi(x' + z, y, u) p(x', x, s) dF(z) dx' ds + e^{-\lambda u} Q(x, y, u) \quad (3.25) $$
...
% \textit{Stochastic Volatility Model:}
% $$ \frac{\partial \Phi}{\partial u} = k(\theta - x)\frac{\partial \Phi}{\partial x} + \kappa(\mu - y)\frac{\partial \Phi}{\partial y} + \frac{1}{2}y \frac{\partial^2 \Phi}{\partial x^2} + \frac{1}{2}\xi^2 y \frac{\partial^2 \Phi}{\partial y^2} + \mathcal{I}(\Phi)(x, y, u) \quad (3.41) $$
...
where $PD_{1yr} = \Phi(x, \dots, 1)$ is calculated from the model.
...
where $PDPiT_i$ is the Point-in-Time PD for period $i$. This is calculated from the default probability function as the probability of defaulting exactly in period $i$, given no default until $i-1$:
$$ PDPiT_i = \Phi(x, \dots, i) - \Phi(x, \dots, i-1) \quad (4.32 \text{ adapted}) $$
...
$$
\mathcal{R}(t, x; \phi) := \frac{\partial \phi}{\partial t} - \left( k(\theta - x)\frac{\partial \phi}{\partial x} + \frac{1}{2}\sigma^2 \frac{\partial^2 \phi}{\partial x^2} + \lambda \mathbb{E}_Z[\phi(t, x+Z) - \phi(t, x)] \right)
$$
...
The total loss function $\mathcal{L}(\mathbf{\Theta})$ is composed of three distinct terms: the loss from the PIDE residual at interior points, the loss from the terminal condition, and the loss from the spatial boundary conditions.
...
$$
\mathcal{L}(\mathbf{\Theta}) = \mathcal{L}_{\text{int}}(\mathbf{\Theta}) + \mathcal{L}_{\text{term}}(\mathbf{\Theta}) + \mathcal{L}_{\text{bound}}(\mathbf{\Theta})
$$
Optionally, weights $\alpha_{\text{int}}, \alpha_{\text{term}}, \alpha_{\text{bound}}$ could be introduced to balance the contribution of each term, but they are set to unity in the current implementation. Minimizing $\mathcal{L}(\mathbf{\Theta})$ with respect to the network parameters $\mathbf{\Theta}$ drives the neural network $\hat{\phi}$ to approximate the true solution of the PIDE while satisfying the given terminal and boundary conditions.

\subsection{Training Data Generation}
\label{subsec:data_gen}

Three sets of points are generated for training:
\begin{enumerate}
    \item \textbf{Residual Points:} $N_r = 20000$ collocation points are sampled uniformly from the spatio-temporal domain $[t_{min}, t_{max}] \times [x_{min}, x_{max}]$. These points are used to compute $\mathcal{L}_r$. Gradients are required for these points.
    \item \textbf{Terminal Condition Points:} $N_T = 500$ points are generated at the terminal time $t=t_{max}$. The spatial coordinates $x$ are chosen as linearly spaced values between $x_{min}$ and $x_{max}$ to ensure coverage across the terminal boundary. The corresponding target values are set to 1 if $x <= K$ and 0 otherwise.
    \item \textbf{Boundary Condition Points:} $N_b = 500$ points are generated. Time coordinates $t$ are sampled uniformly from $[t_{min}, t_{max}]$. Half of the points ($N_b/2$) are assigned the spatial coordinate $x=x_{min}$ with a target value of 0, and the other half are assigned $x=x_{max}$ with a target value of 1. The points are shuffled to mix boundary conditions during batch processing.
\end{enumerate}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{training_data_viz.png} % Ensure the image file is accessible
...
\section{Numerical Results} % Example parent section
